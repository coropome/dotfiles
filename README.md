...
workspace backend の default は `tmux` だが、terminal choice を固定したくない時は `ai start --backend stdio` も使える。
...
- Beginner path
  - `./install`
  - `make agent`
  - `ai start` (`tmux` default, `stdio` alternative)
...
`ai start` が最短の入口で、default backend は `tmux`。terminal choice を軽く保ちたい時は `ai start --backend stdio` に切り替えられる。`ai doctor` が最初の診断入口で、`tnew` は tmux を直接使いたい人向けの低レベル入口として残している。

beginner path と workflow chain をまとめて見たい時は `ai --help`、workflow chain だけをざっと見たい時は `ai workflows` を使う。
