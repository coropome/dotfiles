...
[[ "$help_output" == *"backend       tmux default | ai start --backend stdio"* ]] || fail "ai help did not print the backend note"
...
backend_help_line="$(printf '%s\n' "$help_output" | rg '^  backend' -n)"
starter_help_line="$(printf '%s\n' "$help_output" | rg '^  starter repo' -n)"
deeper_help_line="$(printf '%s\n' "$help_output" | rg '^  deeper use' -n)"
[[ -n "$runtime_help_line" && -n "$backend_help_line" && -n "$starter_help_line" && -n "$deeper_help_line" ]] || fail "ai help did not print ordered first-step lines"
[[ "${runtime_help_line%%:*}" -lt "${backend_help_line%%:*}" ]] || fail "ai help did not print runtime guidance before backend guidance"
[[ "${backend_help_line%%:*}" -lt "${starter_help_line%%:*}" ]] || fail "ai help did not print backend guidance before starter guidance"
...
start_help_output="$(PATH="$STUB_BIN:$ORIG_PATH" "$REPO/bin/ai-start" --help)"
[[ "$start_help_output" == *"usage: ai-start [--repo PATH] [--backend <tmux|stdio>] [--stop]"* ]] || fail "ai-start help did not print usage"
[[ "$start_help_output" == *"Workspace backends:"* ]] || fail "ai-start help did not print workspace backend guidance"
[[ "$start_help_output" == *"tmux   current default; durable session with a context pane"* ]] || fail "ai-start help did not describe the tmux default"
[[ "$start_help_output" == *"stdio  lighter non-tmux path when terminal choice should stay flexible"* ]] || fail "ai-start help did not describe the stdio alternative"
[[ "$start_help_output" == *"env override: AI_WORKSPACE_BACKEND=stdio ai start"* ]] || fail "ai-start help did not mention the backend env override"
start_help_tmux_line="$(printf '%s\n' "$start_help_output" | rg '^  tmux' -n)"
start_help_stdio_line="$(printf '%s\n' "$start_help_output" | rg '^  stdio' -n)"
start_help_env_line="$(printf '%s\n' "$start_help_output" | rg '^  env override:' -n)"
[[ -n "$start_help_tmux_line" && -n "$start_help_stdio_line" && -n "$start_help_env_line" ]] || fail "ai-start help did not print ordered backend guidance"
[[ "${start_help_tmux_line%%:*}" -lt "${start_help_stdio_line%%:*}" ]] || fail "ai-start help did not print tmux before stdio"
[[ "${start_help_stdio_line%%:*}" -lt "${start_help_env_line%%:*}" ]] || fail "ai-start help did not print backend choices before the env override"
