...
この doc は local path を置き換えるものではなく、repo-local onboarding が固まったあとに CI/runtime lane を足すための補助線として使う。
...
daily tool として長く使う前提では、pinning は最初から触るものではなく、drift control や fork adoption が必要になった時だけ足す。
...
判断ルールを先に書くと、`local path がまだ不安定なら CI を増やさない`、`PR smoke が必要になったら pr workflow を足す`、`hosted eval と runtime pinning は必要になった時だけ opt-in`。
...
CI/runtime lane を触る前に、local onboarding が `ai doctor -> ai workflows -> ai start` で安定しているかを先に確認する。
ここが不安定なら、この doc より先に local path を直す。
