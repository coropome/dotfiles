...
3. terminal choice を固定したくなく、tmux default path だけを回避したいなら `ai start --backend stdio`
...
- `ai start` の tmux default path だけで詰まるが、workflow / prompt / trust 側は進めたい
  - `ai start --backend stdio`
  - `AI_WORKSPACE_BACKEND=stdio ai start`
  - tmux 自体の異常を追う時だけ `make doctor`
...
- tmux が起動しない
  ...
  - AI workflow 側を先に進めたいだけなら `ai start --backend stdio`
